...
function update(relativePath, before, after, alreadyApplied) {
  const filePath = path.join(root, relativePath)
  const source = fs.readFileSync(filePath, "utf8").replace(/\r\n/g, "\n")

  if (source.includes(after) || (alreadyApplied && source.includes(alreadyApplied))) {
    return
  }
...
update(
  ".quartz/plugins/content-index/src/emitter.ts",
  `        delete content.date;`,
  `        // Keep the display date in the browser index for Explorer sorting.`,
)

update(
  ".quartz/plugins/content-index/src/emitter.ts",
  `toHtml(tree as Root, { allowDangerousHtml: true })`,
  `toHtml(tree as unknown as Parameters<typeof toHtml>[0], { allowDangerousHtml: true })`,
)
...
update(
  ...
  `  sortFn: (a: FileTrieNode, b: FileTrieNode) => {
    if (!a.isFolder && !b.isFolder) {
      const aDate = Date.parse(String(a.data?.date ?? ""));
      const bDate = Date.parse(String(b.data?.date ?? ""));
...
update(
  ".quartz/plugins/explorer/src/components/Explorer.tsx",
  `    if (!a.isFolder && !b.isFolder) {`,
  `    const aIsRecentUpdates = !a.isFolder && a.slugSegment === "recent-updates";
    const bIsRecentUpdates = !b.isFolder && b.slugSegment === "recent-updates";
    if (aIsRecentUpdates !== bIsRecentUpdates) return aIsRecentUpdates ? 1 : -1;
    if (!a.isFolder && !b.isFolder) {`,
)
...
update(
  ...
  `      if (options.includeEmptyFiles || slug === "recent-updates" || (text && text !== "")) {`,
)

// This navigation page belongs in Explorer, but not in the root article listing.
update(
  ".quartz/plugins/folder-page/src/components/FolderContent.tsx",
  `            .filter((page) => page.unlisted !== true)`,
  `            .filter((page) => page.unlisted !== true && page.slug !== "recent-updates")`,
)
...
update(
  ...
  `              pageSlug !== "index-list" &&`,
  `              pageSlug !== "index-list" &&
              pageSlug !== "recent-updates" &&`,
)

for (const plugin of ["content-index", "explorer", "folder-page", "tag-page", "content-meta"]) {
  buildPlugin(plugin)
}
